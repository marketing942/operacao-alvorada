...
(function () {
  ...
  var CONFIG = {
    ...
    //
    // 🔴 OBRIGATÓRIO ficar true nesta landing. Ela roda o Modelo A
    // (TRACKING.md §14): quem emite o Lead é a regra de submit do painel da
    // PixelX. Sem este corte, um cadastro de comunidade gratuita entraria na
    // mesma conversão da venda da cadeira e estragaria a otimização.
    stopSubmitPropagation: true,

    // Trava extra: o popup não pode nascer por cima do modal de checkout —
    // dois diálogos abertos ao mesmo tempo, e o de saída roubando a atenção
    // de quem já estava preenchendo os dados para pagar.
    blockWhen: function () {
      var checkout = document.getElementById("modal");
      return !!(checkout && checkout.classList.contains("is-open"));
    }
  };
